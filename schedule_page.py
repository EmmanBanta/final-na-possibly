...
def normalize_location(name):
    # Normalize area names: lowercase, replace hyphens/underscores with space, strip whitespace
...
# POST: Save or update a schedule
...
def set_schedule(city, barangay, location):
    ...
    # Format and validate input schedule
    formatted_schedule = {}
...
# GET: Fetch schedule for a location
...
def get_schedule(city, barangay, location):
    ...
    try:
        schedule = ref.get()
        if not schedule:
            return jsonify({})  # Return empty object for no schedule (to avoid 404 loop)
